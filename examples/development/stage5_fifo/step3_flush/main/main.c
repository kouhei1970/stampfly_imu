/**
 * @file main.c
 * @brief BMI270 Step 3: FIFO Flush and Stream Mode
 *
 ...
 * - FIFO flush functionality (CMD 0xB0) to clear FIFO
 * - Auto-recovery from skip frames by flushing FIFO
...
static const char *TAG = "BMI270_STEP3";
...
static esp_err_t fifo_flush(void)
{
    ...
    if (ret == ESP_OK) {
        g_flush_count++;
        ESP_LOGW(TAG, "FIFO flushed (count: %lu)", g_flush_count);
 ...
 * @return true if skip frame detected (need flush), false otherwise
 */
static bool parse_fifo_buffer(const uint8_t *buffer, uint16_t length)
{
    ...
    for (int i = 0; i < num_frames; i++) {
        ...
        if (frame[0] == FIFO_HEADER_SKIP) {
            skip_detected = true;
            g_skip_frames++;
            ESP_LOGW(TAG, "Skip frame detected at position %d/%d", i+1, num_frames);
            continue;
        }
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, " BMI270 Step 3: FIFO Flush and Stream Mode");
    ESP_LOGI(TAG, "========================================");
    ...
    // Step 6: Start FIFO multi-frame read loop with flush support
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, " FIFO Multi-Frame Read Loop with Flush");
    ESP_LOGI(TAG, "========================================");
    ...
    while (1) {
        ...
        if (fifo_length >= FIFO_FRAME_SIZE_HEADER) {
            ...
            bool skip_detected = parse_fifo_buffer(fifo_buffer, fifo_length);

            // If skip frame detected, flush FIFO to recover
            if (skip_detected) {
                ESP_LOGW(TAG, "Skip frame detected - flushing FIFO to recover");
                ret = fifo_flush();
                if (ret != ESP_OK) {
            ...
            ESP_LOGI(TAG, "FIFO length after read: %u bytes", fifo_length_after);

            // Statistics
            ESP_LOGI(TAG, "Statistics: Total=%lu Valid=%lu Skip=%lu Config=%lu Flush=%lu",
                     g_total_frames, g_valid_frames, g_skip_frames, g_config_frames, g_flush_count);
        }
