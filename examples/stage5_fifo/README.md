...
- **設定値**: 512バイト
- **フレーム数**: 512 ÷ 13 ≈ 39フレーム
- **データ取得頻度**: 100Hz ÷ 39 ≈ 2.6Hz（約390msごと）
- **SPI通信回数**: 約**2.6回/秒**（vs. Stage 4の100回/秒）

**注意**: ODRが200Hzの場合は、200Hz ÷ 39 ≈ 5.1Hz（約196msごと）になります。

## 性能比較

| 項目 | Stage 3<br>(Polling) | Stage 4<br>(Interrupt) | Stage 5<br>(FIFO @100Hz) |
|------|---------------------|----------------------|------------------|
| CPU使用率 | 高 | 低 | 最低 |
| SPI通信回数/秒 | 100回 | 100回 | **2.6回** |
| 割り込み回数/秒 | - | 100回 | **2.6回** |
| データ遅延 | 最大10ms | < 10µs | 最大390ms |
| データ損失リスク | 低 | 低 | 最低（FIFO保護） |
| 適用用途 | 低速アプリ | リアルタイム | 高速ロギング |

**注意**: 上記は100Hz ODR設定の場合。200Hz ODRではSPI通信回数は約5.1回/秒になります。
